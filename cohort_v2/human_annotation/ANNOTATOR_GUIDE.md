# Human Kappa Annotation Guide (Annotator Edition, Double-Blind) - v2 Cohort

> English rendering prepared for public release.
> **Cohort:** v2 formal rerun (`cohort_v2` here); its generated text is **not** the same batch as v1.
...
| anything under the v1 directory (`cohort_v1/`) | no (avoid comparing against old answers) |

**Important:** even when a record id matches the v1 kappa sample, the v2 record was generated fresh;
**never** copy v1 annotation content.
...
Packet files are named like `pkt-001.json`; CSV rows are keyed by `MAIN-xxxx`. The analyst resolves
identities via the manifest - annotators never need to know domain or trajectory type.
...
- **36** MAIN trajectories (3 domains x 3 trajectory types x 4 records, stratified)
- Same record-id list as v1, but text content comes from the v2 formal run
